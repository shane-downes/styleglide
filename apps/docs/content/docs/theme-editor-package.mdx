...
1. A button is loaded into your app that activates the theme editor
2. StyleGlide generates shadcn/ui CSS themes on demand
3. Changes are are dynamically injected into the DOM
4. Font faces are automatically loaded
5. Your application updates with a real-time preview
6. You can copy the current theme or install it with the shadcn CLI
...
- Checks `dark` class on `<html>` for mode toggle integration
...
export function ThemeEditor() {
  useEffect(() => {
    themeEditor({
      enabled: process.env === "development", // [!code highlight]
    });
  }, []);
...
If your app has a `system` mode option, make sure to pass in the **resolved** mode of `light` or `dark`.
